...
def query_best_hyperparameter(args):
    ...
    if os.path.exists(json_path):
        ...
        best_hparam = best_result[1]

        with open(json_path, 'w') as f:
            json.dump({
                'lr_mul': best_hparam[0],
                'epochs_mul': best_hparam[1],
                'grad_acc_mul': best_hparam[2],
                'loss_valid': best_result[0],
                'search_space': cases,
                'args': args,
                'raw_results': results,
            }, f, indent=2)
        
        print(f'Main.query_best_hparam: Find the best hparam {best_result}')
